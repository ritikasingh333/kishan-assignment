...
class Api extends MX_Controller {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($patient == 'patient_id') {
                ...
                if (empty($patient_exist)) {
                    $this->session->set_flashdata('feedback', lang('invalid_patient_id'));
                    redirect("frontend");
                }
            }
            ...
            if ($patient == 'add_new') {
                ...
                $message = 'Dear ' . $p_name . ', Thank you for the registration. <br> Here is your login details.<br> <br> Link: ' . base_url() . 'auth/login <br> Username: ' . $p_email . ' <br> Password: ' . $password . '<br><br> Thank You, <br>' . $this->settings->title;
            ...
            if (empty($id)) {     // Adding New department
                ...
                $this->session->set_flashdata('feedback', lang('appointment_added_successfully_please_wait_you_will_get_a_confirmation_sms'));
            }
    ...
    public function update() {
        ...
        if (!empty($email)) {
            ...
            // Validating Currency Field   
            $this->form_validation->set_rules('twitter_id', 'Twitter Id', 'trim|min_length[1]|max_length[100]|xss_clean');
            // Validating Currency Field   
        ...
        if (!empty($settings->logo)) {
            if (file_exists($settings->logo)) {
               $data['logo'] = $settings->logo;
                ...
                $data['title'] = $title[0] . '<span> ' . $title[1] . '</span>';
            }
        } else {
             $data['title'] = $title[0] . '<span> ' . $title[1] . '</span>';
        }
